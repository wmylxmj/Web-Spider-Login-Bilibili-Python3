...
class Crack():
    ...
    def crack(self, try_time=5):
        '''log in'''
        self.open_webpage()
        self.login_bilibili()
        for i in range(try_time):
            try:
                gapbg_location_list, fullbg_location_list = self.get_identifying_images(show_images=True)
                gapbg_image = self.merge_identifying_image(bili.gapbg_filename, gapbg_location_list, show_images=True)
                fullbg_image = self.merge_identifying_image(bili.fullbg_filename, fullbg_location_list, show_images=True)
                ...
                print('You have logged in or an error happened')
                pass
            pass
        pass
